...
public class UserDialog implements Runnable{
	...
	public void run() {
		...
		JComboBox<Object> cb = new JComboBox<Object>(cbItems);
	...
	public void login(String email, char[] pw) {
		...
		if(logError != true){
			...
			String ok = fetchPush.login(email, pwString);
			if (ok != "error"){
				User usr = new User(Integer.parseInt(ok));
				CalendarFrame ca = new CalendarFrame();
				System.out.println("We've tried open calendar 1");
				ca.run();
				System.out.println("We've tried open calendar 2");
				//frame.setVisible(false);
				frame.dispose();
				System.out.println("We've tried open calendar");
			}
		}
	...
	public void signup(){
		...
		if(error != true){
			String pwString = new String(passwdTF.getPassword());
			String ok = fetchPush.signup(fnameTF.getText(),
							lnameTF.getText(),
							phoneNumTF.getText(),
							emailTF.getText(),
							pwString);
